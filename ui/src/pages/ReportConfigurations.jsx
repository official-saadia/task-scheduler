...
export default function ReportConfigurations() {
  const [editingConfig, setEditingConfig] = useState(null)
  const [actionError, setActionError] = useState('')

  const fetchFn = useCallback((page, size) => reportsApi.getAll(page, size), [])
  const { data: configs, loading, error, refresh, ...pagination } = useServerPagination(fetchFn, 10)

  async function handleDeactivate(id) {
    if (!window.confirm('Deactivate this report configuration?')) return
    try {
      await reportsApi.deactivate(id)
      refresh()
    } catch (err) {
  ...
  return (
    <div>
      <PageHeader
        ...
        description="Shell commands Task Scheduler runs to generate reports. Point an email task's attachment path at the output file to email the result."
        action={<PrimaryButton onClick={() => setEditingConfig({})}><Plus size={16} /> New Configuration</PrimaryButton>}
      />
      <ErrorBanner message={error || actionError} />
      {configs.length === 0 && !loading ? (
        <EmptyState title="No report configurations yet" description="Add one before creating report generation tasks." />
      ) : (
        <>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-[color:var(--color-border-soft)] bg-[color:var(--color-surface)] text-left text-xs uppercase text-[color:var(--color-ink-muted)]">
                  ...
                  <th className="px-4 py-3 font-medium">Output File</th>
              ...
              <tbody>
                {configs.map((c) => (
                  <tr key={c.id} className="border-b border-[color:var(--color-border-soft)] last:border-0 odd:bg-white even:bg-[color:var(--color-row-alt)] hover:bg-[color:var(--color-status-scheduled-soft)] transition-colors">
                    ...
                    <td className="px-4 py-3 font-[var(--font-mono)] text-xs max-w-xs truncate" title={c.outputFilePath}>{c.outputFilePath}</td>
      ...
      {editingConfig !== null && (
        <ReportFormModal config={editingConfig} onClose={() => setEditingConfig(null)} onSaved={() => { setEditingConfig(null); refresh() }} />
      )}
    </div>
  )
}

function ReportFormModal({ config, onClose, onSaved }) {
  ...
  const [outputFilePath, setOutputFilePath] = useState(config.outputFilePath || '')
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      isEditing ? await reportsApi.update(config.id, payload) : await reportsApi.create(payload)
      onSaved()
    } catch (err) {
  ...
  return (
    <Modal title={isEditing ? 'Edit Report Configuration' : 'New Report Configuration'} onClose={onClose}>
      <form onSubmit={handleSubmit}>
        <ErrorBanner message={error} />
        <Field label="Name"><Input value={name} onChange={(e) => setName(e.target.value)} placeholder="Weekly DLQ report" required /></Field>
        <Field
          label="Command"
          hint="Task Scheduler only runs this command and checks its exit code — it does not know how the report itself is built. This can call the app's own APIs (e.g. GET /api/v1/task-dlq) or query the database directly."
        >
          <Textarea rows={3} value={command} onChange={(e) => setCommand(e.target.value)} placeholder="/opt/scripts/dlq_report.sh" required />
        </Field>
        <Field label="Output file path" hint="Task Scheduler checks this file exists after the command runs, and treats a missing file as a failure even if the command exited 0.">
          <Input value={outputFilePath} onChange={(e) => setOutputFilePath(e.target.value)} placeholder="/reports/dlq_report.csv" required />
        </Field>
